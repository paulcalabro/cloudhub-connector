...
import org.mule.modules.cloudhub.connectivity.CloudHubConnectorConfigConnectionManagementConnectionManager;
...
public class CloudHubConnectorBasicAuthConfigConfigDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(CloudHubConnectorBasicAuthConfigConfigDefinitionParser.class);
    ...
    public BeanDefinition parse(Element element, ParserContext parserContext) {
        ...
        setInitMethodIfNeeded(builder, CloudHubConnectorConfigConnectionManagementConnectionManager.class);
        setDestroyMethodIfNeeded(builder, CloudHubConnectorConfigConnectionManagementConnectionManager.class);
        parseProperty(builder, element, "maxWaitTime", "maxWaitTime");
        parseProperty(builder, element, "username", "username");
        parseProperty(builder, element, "password", "password");
        parseProperty(builder, element, "url", "url");
        parseProperty(builder, element, "sandbox", "sandbox");
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(CloudHubConnectorConfigConnectionManagementConnectionManager.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the configuration [config] within the connector [cloudhub] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the configuration [config] within the connector [cloudhub] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
