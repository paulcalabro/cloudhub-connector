...
import org.mule.modules.cloudhub.adapters.CloudHubConnectorConnectionManagementAdapter;
import org.mule.modules.cloudhub.config.BasicAuthConfig;
import org.mule.modules.cloudhub.pooling.DevkitGenericKeyedObjectPool;


/**
 * A {@code CloudHubConnectorConfigConnectionManagementConnectionManager} is a wrapper around {@link CloudHubConnector } that adds connection management capabilities to the pojo.
 * 
 */
@SuppressWarnings("all")
@Generated(value = "Mule DevKit Version 3.7.2", date = "2015-11-24T09:26:58-03:00", comments = "Build 3.7.x.2633.51164b9")
public class CloudHubConnectorConfigConnectionManagementConnectionManager
    extends ExpressionEvaluatorSupport
    implements MetadataAware, MuleContextAware, ProcessAdapter<CloudHubConnectorConnectionManagementAdapter> , Capabilities, Disposable, Initialisable, Testable, ConnectionManagementConnectionManager<ConnectionManagementConfigCloudHubConnectorConnectionKey, CloudHubConnectorConnectionManagementAdapter, BasicAuthConfig>
{
    ...
    public CloudHubConnectorConnectionManagementAdapter acquireConnection(ConnectionManagementConfigCloudHubConnectorConnectionKey key)
        throws Exception
    {
        return ((CloudHubConnectorConnectionManagementAdapter) connectionPool.borrowObject(key));
    }

    public void releaseConnection(ConnectionManagementConfigCloudHubConnectorConnectionKey key, CloudHubConnectorConnectionManagementAdapter connection)
        throws Exception
    {
        connectionPool.returnObject(key, connection);
    }

    public void destroyConnection(ConnectionManagementConfigCloudHubConnectorConnectionKey key, CloudHubConnectorConnectionManagementAdapter connection)
        throws Exception
    {
    ...
    @Override
    public<P >ProcessTemplate<P, CloudHubConnectorConnectionManagementAdapter> getProcessTemplate() {
        return new ConnectionManagementProcessTemplate(this, muleContext);
    }

    @Override
    public ConnectionManagementConfigCloudHubConnectorConnectionKey getDefaultConnectionKey() {
        return new ConnectionManagementConfigCloudHubConnectorConnectionKey(getUsername(), getPassword(), getUrl(), getSandbox());
    }

    @Override
    public ConnectionManagementConfigCloudHubConnectorConnectionKey getEvaluatedConnectionKey(MuleEvent event)
        throws Exception
    {
        if (event!= null) {
            ...
            final String _transformedSandbox = ((String) evaluateAndTransform(muleContext, event, this.getClass().getDeclaredField("sandbox").getGenericType(), null, getSandbox()));
            return new ConnectionManagementConfigCloudHubConnectorConnectionKey(_transformedUsername, _transformedPassword, _transformedUrl, _transformedSandbox);
        }
    ...
    @Override
    public ConnectionManagementConfigCloudHubConnectorConnectionKey getConnectionKey(MessageProcessor messageProcessor, MuleEvent event)
        throws Exception
    {
    ...
    public ConnectionManagementConnectionAdapter newConnection() {
        BasicAuthConfigCloudHubConnectorAdapter connection = new BasicAuthConfigCloudHubConnectorAdapter();
        connection.setMaxWaitTime(getMaxWaitTime());
        return connection;
    }

    @Override
    public ConnectionManagementConnectorAdapter newConnector(ConnectionManagementConnectionAdapter<BasicAuthConfig, ConnectionManagementConfigCloudHubConnectorConnectionKey> connection) {
        CloudHubConnectorConnectionManagementAdapter connector = new CloudHubConnectorConnectionManagementAdapter();
    ...
    public ConnectionManagementConnectionAdapter getConnectionAdapter(ConnectionManagementConnectorAdapter adapter) {
        CloudHubConnectorConnectionManagementAdapter connector = ((CloudHubConnectorConnectionManagementAdapter) adapter);
    ...
    public TestResult test() {
        try {
            BasicAuthConfigCloudHubConnectorAdapter strategy = ((BasicAuthConfigCloudHubConnectorAdapter) newConnection());
